...
puzzle_input.each do |_puzzle_key, individual_puzzle|
  ...
  individual_puzzle.each do |key, value|
    ...
    if potential_matches.size > 1
      ...
      (1..value.size).each do |n|
        unmatched_segments[value.size][counts_per_size[value.size]] << value[n - 1]
      end
    ...
    (1..value.size).each do |n|
      numbers_to_segments[potential_matches.keys.first][:segment_wires] << value[n - 1]
    end
  end
  ...
  unmatched_segments[6].each do |key, value|
    ...
    numbers_to_segments[9][:segment_wires] = []
    (1..value.size).each do |n|
      numbers_to_segments[9][:segment_wires] << value[n - 1]
    end
  ...
  unmatched_segments[6].each do |key, value|
    ...
    numbers_to_segments[number][:segment_wires] = []
    (1..value.size).each do |n|
      numbers_to_segments[number][:segment_wires] << value[n - 1]
    end
  end
  ...
  unmatched_segments[5].each do |key, value|
...
# rubocop:enable Style/PercentLiteralDelimiters, Style/StringLiterals, Layout/LineLength, Metrics/BlockLength, Lint/Debugger, Metrics/MethodLength
puts "Part 2: Sum of all 4 digit output values: #{output_values.sum}"